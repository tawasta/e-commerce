        ...
        if all_lines:
            ...
            for record in new_lines:
                if website.company_id != record.product_id.company_id:
                    ...
                    fpos = record.order_id.fiscal_position_id or record.order_id.fiscal_position_id.get_fiscal_position(record.order_partner_id.id)
                    # If company_id is set, always filter taxes by the company
                    taxes = record.product_id.taxes_id.filtered(lambda t: t.company_id == record.env.company)
                    tax_id = fpos.map_tax(taxes, record.product_id, record.order_id.partner_shipping_id)
