...
class WebsiteEventSale(WebsiteSale):
    ...
    def shop_payment(self, **post):
        ...
        if order and post:
            ...
            transaction = order.transaction_ids.filtered(lambda t: t.state in ('draft', 'pending'))
            _logger.info("===TRANSACTION=== %s", transaction)
            if transaction:
                payment_method = transaction[0].payment_method_id
                product = payment_method.product_id
                if product and not any(line.product_id.id == product.id for line in order.order_line):
