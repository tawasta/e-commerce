    ...
    def _send_email(self):
        ...
        for wizard_line in self:
            ...
            if wizard_line.wizard_id.sale_order_id:
                product_categories = wizard_line.wizard_id.sale_order_id.order_line.mapped('product_id.categ_id')
                while product_categories:
                    for category in product_categories:
                        if category.create_user_attachment and category.allow_create_user:
            ...
            if template:
                template.with_context(**context).send_mail(wizard_line.id, force_send=True, email_values=email_values)
            else:
