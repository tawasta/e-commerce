odoo.define('website_sale_company_slider.checkout', function (require) {
    ...
    $(function() {
        ...
        function showFields() {
            ...
            $("label[for='company_name']").toggleClass('label-optional font-weight-normal', !is_company);
            $("label[for='company_email']").toggleClass('label-optional font-weight-normal', !is_company);

            // Toggle field visibility by customer type
            $("input[name='company_name']").parent().toggleClass('d-none', !is_company);
            $("input[name='company_email']").parent().toggleClass('d-none', !is_company);
            ...
            if (is_company === true) {
                if (required_fields.val().indexOf(',company_name,vat') < 0) {
                    required_fields.val(required_fields.val() + ',company_name,company_email,vat');
                }
            } else {
                required_fields.val(required_fields.val().replace(',company_name,company_email,vat', ''));
            }
        }
